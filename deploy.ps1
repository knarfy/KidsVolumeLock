...
if ($devices -match "device$") {
    Write-Host "[OK] Tablet detectada" -ForegroundColor Green
}
else {
    Write-Host "[ERROR] No se detectó ninguna tablet. Conecta la tablet y permite USB debugging." -ForegroundColor Red
    exit 1
}
...
if ($status) {
    ...
    git add .
    $commitMsg = "Update from automated script"
...
while ($attempt -lt $maxAttempts -and -not $buildComplete) {
    ...
    # Check if run exists
    if ($runJson) {
        $status = $runJson[0].status
        $conclusion = $runJson[0].conclusion
        ...
        elseif ($conclusion -eq "failure") {
            Write-Host "[ERROR] Build falló. Revisa GitHub Actions." -ForegroundColor Red
            exit 1
        }
        else {
...
if (-not $buildComplete) {
    Write-Host "[ERROR] Timeout esperando el build" -ForegroundColor Red
    exit 1
}
...
if (Test-Path ".\build_output\app-debug\app-debug.apk") {
    ...
    Write-Host "[OK] APK descargada ($([math]::Round($apkSize, 2)) MB)" -ForegroundColor Green
}
else {
    Write-Host "[ERROR] No se encontró la APK en los artefactos" -ForegroundColor Red
    exit 1
}
...
Write-Host "[OK] Versión anterior desinstalada (si existía)" -ForegroundColor Green

# 6. Instalar nueva APK
Write-Host ""
Write-Host "[6/6] Instalando nueva APK en la tablet..." -ForegroundColor Yellow
$installResult = & $adbPath install -r .\app-debug-latest.apk 2>&1
if ($installResult -match "Success") {
    Write-Host "[OK] APK instalada exitosamente" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "[ERROR] Error al instalar: $installResult" -ForegroundColor Red
    exit 1
}
